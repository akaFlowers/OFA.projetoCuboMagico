...
BEGIN
SELECT Usuario.usuario, Usuario.senha, Usuario.nivelAcesso ,Cliente.id, Cliente.nome, Cliente.sobreNome, Cliente.dataNascimento, Cliente.sexo, Cliente.tamCamiseta, Cliente.cpf, Cliente.email, Cliente.telefone, Cliente.celular, Cliente.cep, Cliente.estado, Cliente.cidade, Cliente.bairro, Cliente.rua, Cliente.numero, Cliente.complemento, Cliente.pais, Cliente.idUsuario FROM Cliente JOIN Usuario ON Cliente.id = Usuario.id;
END $$
DELIMITER ;

DELIMITER $$
DROP PROCEDURE IF EXISTS SP_consultaClientePorID $$
CREATE PROCEDURE SP_consultaClientePorID(IN ID INT)
BEGIN
SELECT Usuario.usuario, Usuario.senha, Usuario.nivelAcesso, Cliente.id, Cliente.nome, Cliente.sobreNome, Cliente.dataNascimento, Cliente.sexo, Cliente.tamCamiseta, Cliente.cpf, Cliente.email, Cliente.telefone, Cliente.celular, Cliente.cep, Cliente.estado, Cliente.cidade, Cliente.bairro, Cliente.rua, Cliente.numero, Cliente.complemento, Cliente.pais, Cliente.idUsuario FROM Cliente JOIN Usuario ON Cliente.id = Usuario.id
WHERE Cliente.id = ID;
END $$
...
BEGIN
SELECT Livro.id, Livro.nome, Livro.autor, Livro.idGeneroLivro , GeneroLivro.generoLivro, GeneroLivro.subGenero, Livro.dataPublicacao, Livro.editora FROM Livro JOIN GeneroLivro ON Livro.id = GeneroLivro.id;   
END $$
DELIMITER ;

DELIMITER $$
DROP PROCEDURE IF EXISTS SP_consultaLivroPorID $$
CREATE PROCEDURE SP_consultaLivroPorID(IN ID INT)
BEGIN
SELECT Livro.id, Livro.nome, Livro.autor, Livro.idGeneroLivro , GeneroLivro.generoLivro, GeneroLivro.subGenero, Livro.dataPublicacao, Livro.editora FROM Livro JOIN GeneroLivro ON Livro.id = GeneroLivro.id
WHERE Livro.id = ID;
END $$
